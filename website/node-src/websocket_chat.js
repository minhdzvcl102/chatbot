  ...
  wss.on("connection", function connection(ws) {
    ...
    ws.on("message", async function incoming(message) {
      ...
      let aiResponseContent =
        "Đã xảy ra lỗi không xác định khi xử lý tin nhắn."; // Giá trị mặc định

      try {
        const parsedMessage = JSON.parse(receivedMessage);

        // Giả định tin nhắn từ client có dạng { type: 'user_message', content: '...' }
        if (parsedMessage.type === "user_message" && parsedMessage.messages) {
          // Nếu có mảng messages, truyền sang Flask
          const pythonServiceURL = `${process.env.PYTHON_SERVICE_URL}/data/process`;
          ...
          if (pythonResponse.data && pythonResponse.data.ai_response) {
            ...
            aiResponseContent = `Dịch vụ AI trả về định dạng không mong muốn: ${JSON.stringify(
              pythonResponse.data
            )}`;
          }
        } else if (
          ...
          // Fallback: chỉ có content
          const pythonServiceURL = `${process.env.PYTHON_SERVICE_URL}/data/process`;
          const pythonResponse = await axios.post(pythonServiceURL, {
            user_query: parsedMessage.content,
          });
          if (pythonResponse.data && pythonResponse.data.ai_response) {
            aiResponseContent = pythonResponse.data.ai_response;
          } else {
          ...
          parsedMessage.type === "file_message" &&
          parsedMessage.file_content
        ) {
          const pythonResponse = await axios.post(
            `${process.env.PYTHON_SERVICE_URL}/file/process`,
            {
              file_content: parsedMessage.file_content, // Giả sử file_content là base64 hoặc nội dung file
            }
          );
          aiResponseContent = `Đã nhận file : ${JSON.stringify(
            pythonResponse.data
          )}`;
        } else {
          aiResponseContent =
            "Đã nhận được tin nhắn không đúng định dạng hoặc thiếu nội dung.";
        }
      } catch (error) {
        console.error("LỖI XỬ LÝ TRÊN WEBSOCKET SERVER:", error);
        if (error.response) {
          // Lỗi từ server Python (status code 4xx, 5xx)
          aiResponseContent = `Lỗi từ AI Service (HTTP Status: ${
            error.response.status
          }): ${JSON.stringify(error.response.data)}`;
        } else if (error.request) {
          // Không có phản hồi từ server (ví dụ: server Flask chưa chạy, lỗi mạng, CORS)
          aiResponseContent =
            "Không nhận được phản hồi từ AI Service. Có thể server Python chưa chạy hoặc lỗi mạng/CORS.";
        } else {
        ...
        try {
          ws.send(
            JSON.stringify({ type: "ai_response", content: aiResponseContent })
          );
        } catch (sendError) {
    ...
    ws.send(
      JSON.stringify({
        type: "system_message",
        content: "Chào mừng bạn đến với chatbot!",
      })
    );
  });
};
